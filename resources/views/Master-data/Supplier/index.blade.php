        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 gap-3">
                <div class="flex gap-3">
                    <a href="{{ route('supplier.create') }}" class="bg-blue-600 text-white px-3 py-2 rounded-lg shadow hover:bg-blue-700 text-sm flex items-center">
                        + Tambah Supplier
                    </a>
                </div>
            </div>
                            ...
                            <td class="px-3 py-2 border text-center">
                                ...
                                @endif
                                    ...
                                    <td class="px-3 py-2 border">
                                        <a href="{{ route('supplier.edit', $item->SupplierID) }}" class="bg-yellow-500 text-white px-2 py-1 rounded hover:bg-yellow-600 text-xs inline-block">Edit</a>
                                        <form action="{{ route('supplier.destroy', $item->SupplierID) }}" method="POST" class="inline-block" onsubmit="return confirm('Yakin hapus?');">
                                            @csrf
                                            @method('DELETE')
