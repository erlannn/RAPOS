                    ...
                    <table class="min-w-full border border-gray-200 text-sm">
                        <thead class="bg-gray-100">
                            <tr>
                                ...
                                <th class="px-3 py-2 border">BrandID</th>
<th class="px-3 py-2 border">KategoriID</th>
<th class="px-3 py-2 border">SubKategoriID</th>
<th class="px-3 py-2 border">SKU</th>
<th class="px-3 py-2 border">Barcode</th>
<th class="px-3 py-2 border">NamaProduk</th>
                        ...
                        <tbody>
                            @foreach ($produk as $index => $item)
                                <tr>
                                    ...
                                    <td class="px-3 py-2 border">{{ $item->BrandID }}</td>
<td class="px-3 py-2 border">{{ $item->KategoriID }}</td>
<td class="px-3 py-2 border">{{ $item->SubKategoriID }}</td>
<td class="px-3 py-2 border">{{ $item->SKU }}</td>
<td class="px-3 py-2 border">{{ $item->Barcode }}</td>
<td class="px-3 py-2 border">{{ $item->NamaProduk }}</td>
<td class="px-3 py-2 border">{{ $item->Satuan }}</td>
<td class="px-3 py-2 border">{{ $item->Deskripsi }}</td>
                            <td class="px-3 py-2 border text-center">
                                ...
                                @endif
                                    ...
                                    <td class="px-3 py-2 border">
                                        <a href="{{ route('produk.edit', $item->ProdukID) }}" class="bg-yellow-500 text-white px-2 py-1 rounded hover:bg-yellow-600 text-xs inline-block">Edit</a>
                                        <form action="{{ route('produk.destroy', $item->ProdukID) }}" method="POST" class="inline-block" onsubmit="return confirm('Yakin hapus?');">
                                            @csrf
                                            @method('DELETE')
