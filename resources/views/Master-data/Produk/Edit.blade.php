    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <form action="{{ route('produk.update', $produk->ProdukID) }}" method="POST" class="space-y-4">
                        @csrf
                        @method('PUT')
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4" x-data="{ 
                            departemen: '{{ $produk->KategoriID ? $kategoris->firstWhere('KategoriID', $produk->KategoriID)?->DepartemenID : '' }}', 
                            kategori: '{{ $produk->KategoriID }}',
                            subkategori: '{{ $produk->SubKategoriID }}',
                            ...
                            <div>
                                ...
                                <select name="BrandID" class="mt-1 block w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500">
                                    ...
                                    @foreach($brands as $brand)
                                        <option value="{{ $brand->BrandID }}" {{ $produk->BrandID == $brand->BrandID ? 'selected' : '' }}>{{ $brand->NamaBrand }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700">Departemen</label>
                                <select x-model="departemen" @change="kategori = ''; subkategori = ''" class="mt-1 block w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500">
                            ...
                            <div>
                                ...
                                <select name="KategoriID" x-model="kategori" @change="subkategori = ''" class="mt-1 block w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500">
                                    <option value="">-- Pilih Kategori --</option>
                                    <template x-for="k in filteredKategoris" :key="k.KategoriID">
                                        <option :value="k.KategoriID" x-text="k.Nama" :selected="k.KategoriID == kategori"></option>
                                    </template>
                                </select>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700">Sub Kategori</label>
                                <select name="SubKategoriID" x-model="subkategori" class="mt-1 block w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500">
                                    <option value="">-- Pilih Sub Kategori --</option>
                                    <template x-for="sk in filteredSubKategoris" :key="sk.SubKategoriID">
                                        <option :value="sk.SubKategoriID" x-text="sk.Nama" :selected="sk.SubKategoriID == subkategori"></option>
                                    </template>
                                </select>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700">SKU</label>
                                <input type="text" name="SKU" value="{{ $produk->SKU }}" class="mt-1 block w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500" placeholder="Masukkan SKU">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700">Barcode</label>
                                <input type="text" name="Barcode" value="{{ $produk->Barcode }}" class="mt-1 block w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500" placeholder="Masukkan Barcode">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700">NamaProduk</label>
                                <input type="text" name="NamaProduk" value="{{ $produk->NamaProduk }}" class="mt-1 block w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500" placeholder="Masukkan NamaProduk">
                            </div>
                            <div>
                            ...
                            <div class="flex items-center mt-6">
                                ...
                                <input type="checkbox" name="StatusAktif" value="1" {{ $produk->StatusAktif == 1 ? 'checked' : '' }} class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded">
